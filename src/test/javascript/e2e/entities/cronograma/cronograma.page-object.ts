...
export default class CronogramaComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('cronograma-heading'));
...
export class CronogramaDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('ciecytApp.cronograma.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-cronograma'));
}
