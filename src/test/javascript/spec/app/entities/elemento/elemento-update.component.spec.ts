/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';

import AlertService from '@/shared/alert/alert.service';
import * as config from '@/shared/config/config';
import ElementoUpdateComponent from '@/entities/elemento/elemento-update.vue';
import ElementoClass from '@/entities/elemento/elemento-update.component';
import ElementoService from '@/entities/elemento/elemento.service';

import FormatoService from '@/entities/formato/formato.service';

import ModalidadService from '@/entities/modalidad/modalidad.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.component('font-awesome-icon', {});

describe('Component Tests', () => {
  describe('Elemento Management Update Component', () => {
    let wrapper: Wrapper<ElementoClass>;
    let comp: ElementoClass;
    let elementoServiceStub: SinonStubbedInstance<ElementoService>;

    beforeEach(() => {
      elementoServiceStub = sinon.createStubInstance<ElementoService>(ElementoService);

      wrapper = shallowMount<ElementoClass>(ElementoUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          alertService: () => new AlertService(store),
          elementoService: () => elementoServiceStub,

          formatoService: () => new FormatoService(),

          modalidadService: () => new ModalidadService()
        }
      });
      comp = wrapper.vm;
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.elemento = entity;
        elementoServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(elementoServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.elemento = entity;
        elementoServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(elementoServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });
  });
});
